            ...
            if (err) {
                ...
                if (result.length > 0) {
                    ...
                    for (let idx = 0; idx < THREADS_AMOUNT; idx += 1) {
                        ...
                        //         console.log(err);
                        //     } else if (info != null) {
                        //         console.log("Connection value: ", info.response.data[0]);
                        //     }
                        // });
                        promises.push(new Promise((resolve) => {
                            // const worker = new Worker('./thread.js', { workerData : {idx,con : JSONStream.stringify(connection)} });
                            const worker = new Worker('./thread.js', { workerData : {idx}});
                            ...
                            // const forked = fork('thread.js',[], [ connection ]);
                            // forked.on('message', (msg) => {
                            //     console.log('Message from child', msg);
                            // });
                            // forked.send({ idx, connection });
                        }))
                    }
